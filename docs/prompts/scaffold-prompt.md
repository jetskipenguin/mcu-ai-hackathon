...
- `.env.example` listing every variable: `COUNTERSIGN`, `PORT`, `SESSION_SECRET`, `LLM_PROVIDER`, `OPENAI_API_KEY`, `OPENAI_MODEL`, `ANTHROPIC_API_KEY`, `ANTHROPIC_MODEL`. `.gitignore` covering `.env`, `data/provenance.jsonl`, `data/credentials.json`, `node_modules`, and any PDF.
